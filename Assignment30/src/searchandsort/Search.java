...
public class Search {
	...
	public static int binarySearch(Superhero[] heros,double input){	
	
		int left = 0;
	    int right = heros.length- 1;
	    while (left <= right) {
	        int mid = left + (right - left) / 2;
	        
			if (heros[mid].getPower() == input) {
	            return 1;
	        }
	        if (heros[mid].getPower() < input) {
	            left = mid + 1;
	...
	public static int binarySearch(Seaman[] fish,String input){	
...
public static int binarySearch2(Seaman[] fish,String input){	
